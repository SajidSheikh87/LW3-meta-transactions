...
contract MetaTokenTransfer is Test {
    ...
    uint8 privateKeyOfUserAddress = 1;
    address userAddress = vm.addr(privateKeyOfUserAddress);
    ...
    function setUp() public {
        ...
        // Have user infinite approve the token sender contract forr transferring 'RandomToken'
        randomTokenContract.approve(
            ...
            // 4+4 = 88 i.e. two hex digits = 1 byte
            // 64 digits = 32 bytes
    ...
    function test_nonceMetatransactions() public {
        ...
        bytes32 messageHash =
            tokenSenderContract.getHash(userAddress, 10 ether, recipientAddress, address(randomTokenContract), nonce);
        bytes32 signedMessageHash = MessageHashUtils.toEthSignedMessageHash(messageHash);

        // Sign a digest with private key, returning (v,r,s)
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(privateKeyOfUserAddress, signedMessageHash);
        // pack v, r, s into 65 bytes
        ...
        tokenSenderContract.transfer(
            userAddress, 10 ether, recipientAddress, address(randomTokenContract), nonce, signature
        );

        // Check the user's balance decreased, and recipient got 10 tokens

        uint256 userBalance = randomTokenContract.balanceOf(userAddress);
        uint256 recipientBalance = randomTokenContract.balanceOf(recipientAddress);
        ...
        bytes32 messageHash2 =
            tokenSenderContract.getHash(userAddress, 10 ether, recipientAddress, address(randomTokenContract), nonce);
        bytes32 signedMessageHash2 = MessageHashUtils.toEthSignedMessageHash(messageHash2);

        (uint8 v2, bytes32 r2, bytes32 s2) = vm.sign(privateKeyOfUserAddress, signedMessageHash2);
        bytes memory signature2 = abi.encodePacked(r2, s2, v2);

        // Have the relayer execute the transaction on behalf of the user
        vm.prank(relayerAddress);
        tokenSenderContract.transfer(
            userAddress, 10 ether, recipientAddress, address(randomTokenContract), nonce, signature2
        );
